...
#from temp.config import device
device=torch.device("cpu")

data = pd.read_csv('result.csv', encoding='utf-8')
label = data['predict'].values
data = data.drop('predict', axis=1).values
m = int(len(data) * 5 / 6)
...
class NeuralNet(nn.Module):
    ...
    def __init__(self, input_node, hidden_layer, hidden_node, drop_prob=0.1):
        ...
        self.output_layer = nn.Sequential(
            nn.Linear(hidden_node, 5),  # multi label classification의 경우 1 대신 분류할 라벨의 수를 적어주세요
